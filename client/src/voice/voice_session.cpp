...
VoiceSession::VoiceSession(QObject* parent)
    : QObject(parent)
{
    m_transmitTimer.setInterval(20);
...
VoiceSession::~VoiceSession() { stop(); }
...
bool VoiceSession::initialize(int inputDeviceId, int outputDeviceId,
    ...
    if (!m_audioEngine.initialize()) { emit error("Failed to initialize audio engine"); return false; }
    if (!m_opusCodec.initialize(bitrate, complexity, fec, dtx)) { emit error("Failed to initialize Opus codec"); return false; }
    return true;
}

void VoiceSession::setSrtpSession(SrtpSession* session) { m_srtpSession = session; }
...
void VoiceSession::setServer(const QString& host, int voicePort) {
    ...
    if (m_resolvedAddr.isNull()) {
        ...
        for (const auto& addr : hostInfo.addresses()) {
            if (addr.protocol() == QAbstractSocket::IPv4Protocol) { m_resolvedAddr = addr; break; }
        }
    }
...
bool VoiceSession::start() {
    ...
    if (!m_udpSocket.bind(QHostAddress::AnyIPv4, 0)) {
        emit error("Failed to bind UDP socket"); return false;
    }
    qDebug() << "[VOICE] UDP bound to port" << m_udpSocket.localPort();
    if (!m_audioEngine.startCapture(m_inputDeviceId)) {
        emit error(QString("Failed to start audio capture: %1").arg(QString::fromStdString(m_audioEngine.lastError())));
        return false;
    }
    if (!m_audioEngine.startPlayback(m_outputDeviceId)) {
        emit error(QString("Failed to start audio playback: %1").arg(QString::fromStdString(m_audioEngine.lastError())));
        m_audioEngine.stopCapture(); return false;
    }
    m_txSequence = 0; m_txTimestamp = 0;
    m_transmitTimer.start(); m_playbackTimer.start();
...
void VoiceSession::stop() {
    ...
    m_transmitTimer.stop(); m_playbackTimer.stop();
    m_audioEngine.stopCapture(); m_audioEngine.stopPlayback();
    m_udpSocket.close();
    { std::lock_guard<std::mutex> lock(m_sourcesMutex); m_sources.clear(); }
...
void VoiceSession::setOpenMic(uint32_t channelId, bool enabled) {
    if (enabled) m_openMicChannels.insert(channelId);
    else         m_openMicChannels.remove(channelId);
    qDebug() << "[VOICE] open mic" << (enabled ? "ON" : "OFF") << "for ch=" << channelId;
}

void VoiceSession::setInputVolume(float v)  { m_audioEngine.setInputVolume(v); }
void VoiceSession::setOutputVolume(float v) { m_audioEngine.setOutputVolume(v); }
...
void VoiceSession::setDevices(int inputDeviceId, int outputDeviceId) {
    ...
    m_audioEngine.stopCapture(); m_audioEngine.stopPlayback();
    if (!m_audioEngine.startCapture(m_inputDeviceId))
        emit error(QString("Failed to restart audio capture: %1").arg(QString::fromStdString(m_audioEngine.lastError())));
    if (!m_audioEngine.startPlayback(m_outputDeviceId))
        emit error(QString("Failed to restart audio playback: %1").arg(QString::fromStdString(m_audioEngine.lastError())));
}

float VoiceSession::inputLevel()  const { return m_audioEngine.inputLevel(); }
float VoiceSession::outputLevel() const { return m_audioEngine.outputLevel(); }
...
void VoiceSession::onTransmitTimer() {
    ...
    // Apply ATC radio DSP chain to outgoing audio when the filter is active.
    if (m_atcRadioEnabled.load(std::memory_order_relaxed))
        m_txAtcFilter.process(pcm, static_cast<int>(AudioEngine::FramesPerBuffer));
    ...
    if (pttActive) targets.insert(m_channelId);
    ...
    for (uint32_t chId : targets) {
        ...
        m_udpSocket.writeDatagram(
            ...
            m_resolvedAddr, static_cast<quint16>(m_voicePort));
    }
    m_txSequence++;
...
void VoiceSession::processReceivedPacket(const QByteArray& data) {
    if (!m_srtpSession || !m_srtpSession->isReady()) {
        qDebug() << "[RX] SRTP not ready, dropping packet"; return;
    }
    auto pkt = VoicePacket::parse(
        reinterpret_cast<const uint8_t*>(data.constData()), static_cast<size_t>(data.size()));
    if (!pkt) { qDebug() << "[RX] Failed to parse voice packet, size=" << data.size(); return; }
    if (pkt->userId == m_userId) return;
    qDebug() << "[RX] Got pkt from user=" << pkt->userId << "seq=" << pkt->sequence
             << "payload=" << pkt->payload.size();
    auto decrypted = m_srtpSession->decrypt(pkt->payload.data(), pkt->payload.size(), pkt->sequence);
    if (decrypted.empty()) { qDebug() << "[RX] SRTP decrypt failed for seq=" << pkt->sequence; return; }
    std::lock_guard<std::mutex> lock(m_sourcesMutex);
    auto& source = m_sources[pkt->userId];
    if (!source) { source = std::make_unique<SourceState>(); source->decoder.initialize(); }
    source->channelId = pkt->channelId;
...
void VoiceSession::onPlaybackTimer() {
...

    {
        ...
        for (auto& [userId, source] : m_sources) {
            ...
            if (settingsIt != m_channelSettings.end() && settingsIt->second.muted) continue;
            ...
            int priority = (settingsIt != m_channelSettings.end()) ? settingsIt->second.priority : 5;
            if (priority < highestActivePriority) highestActivePriority = priority;
        ...
        for (auto& ds : decoded_sources) {
            ...
            if (duckingOn && priority > highestActivePriority) volume *= duckLvl;
    ...
    if (hasAudio) {
        // Apply ATC radio effect to all incoming voices when the filter is active.
        if (m_atcRadioEnabled.load(std::memory_order_relaxed))
            m_rxAtcFilter.process(mixed, AudioEngine::FramesPerBuffer);
