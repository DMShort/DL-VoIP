...
QWidget* SettingsDialog::createVoiceFiltersTab() {
    ...
    m_atcParamWidget = new QWidget(this);
    m_atcParamWidget->setEnabled(false);
    auto* paramForm = new QFormLayout(m_atcParamWidget);
    paramForm->setContentsMargins(8, 8, 8, 8);
    paramForm->setVerticalSpacing(10);

    auto makeRow = [&](const QString& label, const QString& tip, QSlider*& out) {
        auto* row       = new QWidget(m_atcParamWidget);
        auto* rowLayout = new QHBoxLayout(row);
        rowLayout->setContentsMargins(0, 0, 0, 0);

        out = new QSlider(Qt::Horizontal, m_atcParamWidget);
        out->setRange(0, 100);
        out->setValue(50);
        out->setToolTip(tip);

        auto* valLabel = new QLabel("50", m_atcParamWidget);
        valLabel->setMinimumWidth(28);
        valLabel->setAlignment(Qt::AlignRight | Qt::AlignVCenter);
        rowLayout->addWidget(out);
        rowLayout->addWidget(valLabel);

        connect(out, &QSlider::valueChanged, valLabel, [valLabel](int v) {
            valLabel->setText(QString::number(v));
        });
        paramForm->addRow(label, row);
    };

    makeRow("Bandwidth Restriction:",
        "Controls the telephone/nasal EQ effect.\n"
        "0 = wide (100-6000 Hz)  |  50 = classic aviation (300-3000 Hz)  |  100 = extremely thin (600-2000 Hz)",
        m_atcBandwidth);

    makeRow("Compression Squash:",
        "Controls dynamic range levelling.\n"
        "0 = gentle (4:1 / -24 dB)  |  50 = classic (15:1 / -40 dB)  |  100 = brickwall (50:1 / -60 dB)",
        m_atcCompression);
    ...
    connect(m_atcRadioEnabled, &QCheckBox::toggled, this, [this](bool checked) {
        m_atcParamWidget->setEnabled(checked);
        if (m_voiceSession)
            m_voiceSession->setAtcRadioEnabled(checked);
    });

    connect(m_atcBandwidth, &QSlider::valueChanged, this, [this](int v) {
        m_config.voice.atcBandwidth = v;
        if (m_voiceSession) m_voiceSession->setAtcBandwidth(v);
    });
    connect(m_atcCompression, &QSlider::valueChanged, this, [this](int v) {
        m_config.voice.atcCompression = v;
        if (m_voiceSession) m_voiceSession->setAtcCompression(v);
    });
    connect(m_atcGrit, &QSlider::valueChanged, this, [this](int v) {
        m_config.voice.atcGrit = v;
        if (m_voiceSession) m_voiceSession->setAtcGrit(v);
    });
    connect(m_atcSquelch, &QSlider::valueChanged, this, [this](int v) {
        m_config.voice.atcSquelch = v;
        if (m_voiceSession) m_voiceSession->setAtcSquelch(v);
    });
...
void SettingsDialog::loadFromConfig() {
    ...
    // Voice Filters -- load checkbox first (toggled signal enables the param widget)
    if (m_atcRadioEnabled) m_atcRadioEnabled->setChecked(m_config.voice.atcRadioEnabled);
    if (m_atcBandwidth)   m_atcBandwidth->setValue(m_config.voice.atcBandwidth);
    if (m_atcCompression) m_atcCompression->setValue(m_config.voice.atcCompression);
    if (m_atcGrit)        m_atcGrit->setValue(m_config.voice.atcGrit);
    if (m_atcSquelch)     m_atcSquelch->setValue(m_config.voice.atcSquelch);
}
...
void SettingsDialog::saveToConfig() {
    ...
    // Voice Filters (sliders persist live; checkbox + final values flushed here)
    if (m_atcRadioEnabled) m_config.voice.atcRadioEnabled = m_atcRadioEnabled->isChecked();
    if (m_atcBandwidth)   m_config.voice.atcBandwidth   = m_atcBandwidth->value();
    if (m_atcCompression) m_config.voice.atcCompression = m_atcCompression->value();
    if (m_atcGrit)        m_config.voice.atcGrit        = m_atcGrit->value();
    if (m_atcSquelch)     m_config.voice.atcSquelch     = m_atcSquelch->value();
